...
\begin{tikzpicture}
  \draw [->] (-4,0) -- (4,0);
  \draw[red,domain=-3.2:3.2] plot [id=todo_some_id_a, samples=500] function {exp(-(x*2)**2)};
  \draw[blue,domain=-3.2:3.2] plot [id=todo_some_id_b, samples=500] function {(36 - x**2 *(-7 + x**2)**2)/36};
...
\begin{tikzpicture}
  \draw [->] (-3,0) -- (3.5,0) node [left] {$y$};
  \draw [->] (0,-0.5) -- (0,3) node [right] {$x$};
  \draw (-0.2,1) -- (0.2,1) node [right] {\footnotesize $1$};
  \draw[domain=-2:1.3] plot [id=todo_some_id, samples=50] function {exp(x)} node [right] {$e^x$};
  \draw[domain=-1:1.5] plot [id=todo_some_id2] function {1+x} node [right] {$p_1(x)$};
\end{tikzpicture}
