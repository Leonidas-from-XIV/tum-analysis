...
\begin{wrapfigure}{r}[1cm]{.4\textwidth}
  ...
  \begin{tikzpicture}[scale=0.5,y=0.6cm]
    ...
    \draw (4,0.2) -- (4,-0.2) node[below left] {$b$};
    
    % y-Achse
    \draw[->] (0,-1) -- (0,19) node[left] {$x$};
    
    % Landmarkierung
    \foreach \x in {-1,...,18}
      \draw[color=gray] (-5,\x-1) -- (-4.0,\x);
    
    \foreach \x in {-1,...,0,3,4,...,15,18}
    ...
    % Abdriftsfunktion
    \draw[domain=-4:4] plot [id=abdrift, samples=50] function {3*(x-(x**3)/(3*4**2))+8} node[right] {Abdrift};
  \end{tikzpicture}
\end{wrapfigure}
