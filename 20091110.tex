...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \emph{streng monoton wachsend}, wenn \(x_1<x_2 \implies f(x_1) < f(x_2)\)
	\end{itemize}
...
\end{figure}
\todo{correct indentation of figure env}
...
\begin{lstlisting}[language=Pascal,captionpos=b,frame=single,mathescape=true,caption=Codierung des Hornerschemas] 
p := $a_n$;
for k := $n$ - 1 to 0 do
	p := p * $x$ + $a_k$;
print($x$,p);
...
\begin{definition}[Rationale Funktion]
	...
	\]
\end{definition}
\noindent Die Nullstelle des Nenners (\( g(x)=0 \)) nennt man Polstelle. Die Division von Polynomen kann man mit dem Euklidischen Algorithmus durchführen (siehe auch F. L. Bauer, D. Knuth).\todo{bib}
