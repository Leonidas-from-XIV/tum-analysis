...
\begin{align*}
	...
	\implies zw &= |z||w|\left((\cos \varphi \cos \psi - \sin \varphi \sin \psi) + \imag (\cos \varphi \sin \psi + \sin\varphi \cos \psi)\right) \\
...
\begin{align*}
	...
	\text{für } |z| = 1 \implies z^n &= (\cos \varphi + \imag \sin \varphi)^n = \cos \left(n\varphi\right) + \imag \sin \left(n\varphi\right) \text{ (Formel von Moivre)}
\end{align*}
%
...
\begin{example}
	\begin{equation*}
		\omega^6 = 1 \implies \omega = \sqrt[6]{1}
	\end{equation*}
	\begin{center}
...
\begin{gather*}
	...
	\implies z_n = \sqrt[n]{|a|}\left(\cos \frac{\alpha + 2\pi k}{n} + \imag \sin \frac{\alpha + 2\pi k}{n}\right) \text{ mit } k = 0, \ldots, n - 1 \text{ $\ldots$ "`$n$-Wurzeln"'}
\end{gather*}
\begin{note}
	Ist das reelle $\sqrt{}$-ziehen eingebettet?
	\[
		a \in \mathbb{R} \implies \alpha = 0
	\]
	\[
		\begin{aligned}
...
\end{example}
\begin{note}[Parameterdarstellung]
  \begin{itemize}
    \item
...
\begin{itemize}
	...
	\item $a^2 + b^2 = c^2$
	\item falls $c = 1 \leadsto \text{Bogenmaß}$
\end{itemize}
...
\emph{(\textannotation{die Graphen von Sinus und Kosinus sollten bekannt sein})}
...
\end{itemize}
\begin{note}
	Drehen von Vektoren $\leadsto$ Matrizeneinführung $Q = \begin{pmatrix}\cos \alpha & \sin \alpha \\ -\sin \alpha & \cos \alpha\end{pmatrix}$
\end{note}
