...
\begin{example}[Evakuierungspumpe]
  ...
  \begin{enumerate}
    ...
    \item[$\implies$] es existiert ein Infimum $\inf p_n$ $\equals$ Grenzwert
    \item
      \begin{equation*}
        \liminfty{p_n} = \frac{\ds \liminfty{p_{n-1}V}+p_0R}{V+R+Z} \implies
...
\begin{enumerate}
  \item $a_n, b_n$ Nullfolgen $\implies$ $a_n+b_n$ Nullfolge
  \item $a_n$ Nullfolge, $b_n$ beschränkt $\implies$ $a_n\cdot b_n$ Nullfolge
  \item $\left|b_n\right| < \left|a_n\right|$, $a_n$ Nullfolge  $\implies$ $b_n$ Nullfolge
\end{enumerate}
...
\begin{proof}
  ...
  \end{align*}
  $\implies \overline a = \widehat a$, da $\varepsilon$ beliebig klein aber $> 0$
\end{proof}
...
\begin{enumerate}
  ...
  \item harmonische Reihe\\
    ...
    \begin{align*}
      ...
      &= \frac{k+3} 2 \\
      \implies \liminfty[k]{s_{2k+1}} &= \infty
    \end{align*}
    %
    \begin{note}
  ...
  \item Eulersche Zahl $\euler = 2,71828\ldots$
    ...
    \begin{enumerate}
      ...
      \item monoton wachsend
        ...
        \end{equation*}
      \item[$\implies$] konvergent, Grenzwert $\leq 3$
    \end{enumerate}
