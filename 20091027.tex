...
\begin{example}
 \begin{itemize}
  \item $1+2+\ldots+n = \frac n 2 (n+1)$
    \begin{enumerate}
    \item Induktionsanfang: $n_0 = 1$, $1 = \frac 1 2 (1+1) = 1$ \Checkmark
    \item Induktionsschluss: \[\underbrace{1+2+\ldots+n}_{A_n} + (n+1) = \frac 1 2 (n+1) + (n+1) = \frac{n+1}2 (n+2)\] \Checkmark
    \end{enumerate}
  \item $1^2+2^2+\ldots+n^2 > \frac{n^3}3$
    \begin{align*}
      n_0 = 1:\text{\hspace{1cm}} &1 > \frac 1 3 \text{\hspace{1cm}\Checkmark} \\
...
\paragraph{Rechenregeln für Wurzeln} $x,y \in \mathbb{R}$ passend zu $n,m \in\mathbb{N}$
\begin{enumerate}
...
\begin{proposition} Binomischer Lehrsatz:
 \[ x,y \in \mathbb{R}, n \in \mathbb{N} \Rightarrow \sum_{k=0}^n \left( {n \choose k} \;\cdot\; x^{n-k}y^k \right) \]
\end{proposition}
...
%%% TODO: choose metafont symbol

\paragraph{Gründe} Mathe $\sqrt{-1}$, Nachrichtentechnik, \TeX-Metafont Version 1 (Donald Knuth)

unbehagliche Situation $x^2+1=0$, in $\mathbb{R}$ nicht lösbar

\paragraph{intuitiver Zugang} neues Symbol $\imag = \sqrt{-1}$, imaginäre Einheit $\imag$

\begin{definition} komplexe Zahlen:
  \[ a, b \in \mathbb{R}: z = \underbrace{a}_\text{Realteil} + \imag \underbrace{b}_\text{Imaginärteil} \]
  \[ \mathbb{C} = \left\{ z : z = a+\imag b; a,b \in \mathbb{R}, \imag = \sqrt{-1} \right\} \]
\end{definition}

\paragraph{anschauliche Darstellung}
